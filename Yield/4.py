...
def analyze_factor_impact(input_data, lr_model, scaler, features, readable_features):
    ...
    # Compute combined fertilizer contributions
    n_k = input_data['NITROGEN'][0] + input_data['POTASSIUM'][0]
    p_k = input_data['PHOSPHORUS'][0] + input_data['POTASSIUM'][0]
    n_p = input_data['NITROGEN'][0] + input_data['PHOSPHORUS'][0]
    n_p_k = input_data['NITROGEN'][0] + input_data['PHOSPHORUS'][0] + input_data['POTASSIUM'][0]

    # Define feature categories
    climatic_features = ['TEMP', 'RAINFALL']
    agronomic_features = ['SIZE', 'AUI', 'NITROGEN', 'PHOSPHORUS', 'POTASSIUM']
    combined_features = ['N+K', 'P+K', 'N+P', 'N+P+K']

    # Create a dictionary for readable combined features
    combined_readable_features = {
        'N+K': 'Nitrogen + Potassium (N+K) 🌿',
        'P+K': 'Phosphorus + Potassium (P+K) 🌱',
        'N+P': 'Nitrogen + Phosphorus (N+P)  🍀',
        'N+P+K': 'Nitrogen + Phosphorus + Potassium (N+P+K) 🌍'
    }
    ...
    # Add combined fertilizer impact (assuming equal weight distribution)
    for feature, value in zip(combined_features, [n_k, p_k, n_p, n_p_k]):
        contribution = value / 3  # Arbitrary division (as they are not part of the original regression)
        impact_text = {
            'feature': combined_readable_features[feature],
            'contribution': contribution,
            'impact_text': f"{combined_readable_features[feature]} has a <b style='color: {'lightgreen' if contribution > 0 else 'red'};'>{'positive impact📈' if contribution > 0 else 'negative impact📉'}</b> over the predicted yield."
        }
        agronomic_impact.append(impact_text)
...
def analyze_yearly_factor_impact(forecasted_features_df, lr_model, scaler, features, readable_features):
    ...
    # Compute combined fertilizer factors
    n_k = forecasted_features_df['NITROGEN'] + forecasted_features_df['POTASSIUM']
    p_k = forecasted_features_df['PHOSPHORUS'] + forecasted_features_df['POTASSIUM']
    n_p = forecasted_features_df['NITROGEN'] + forecasted_features_df['PHOSPHORUS']
    n_p_k = forecasted_features_df['NITROGEN'] + forecasted_features_df['PHOSPHORUS'] + forecasted_features_df['POTASSIUM']

    # Compute total contributions for combined fertilizers (assuming equal weight distribution)
    total_contributions_combined = {
        'N+K': n_k.sum() / 3,  
        'P+K': p_k.sum() / 3,  
        'N+P': n_p.sum() / 3,  
        'N+P+K': n_p_k.sum() / 3  
    ...
    # Create readable names for combined fertilizers
    combined_readable_features = {
        'N+K': 'Nitrogen + Potassium (N+K) 🌿',
        'P+K': 'Phosphorus + Potassium (P+K) 🌱',
        'N+P': 'Nitrogen + Phosphorus (N+P) 🍀',
    ...
    # Add combined fertilizer impact
    for feature, total_contribution in total_contributions_combined.items():
        impact_text = {
            'feature': combined_readable_features[feature],
            'total_contribution': total_contribution,
            'impact_text': f"{combined_readable_features[feature]} has a <b style='color: {'lightgreen' if total_contribution > 0 else 'red'};'>{'positive impact📈' if total_contribution > 0 else 'negative impact📉'}</b> on yield over the projected years."
        }
        agronomic_impact.append(impact_text)
...
def generate_recommendations(input_df, lr_model, scaler, features, readable_features):
    ...
    contributions = input_scaled[0] * lr_model.coef_  # Compute actual impact

    # Compute combined fertilizer impact (N + P + K)
    n_p_k_contribution = (input_df['NITROGEN'][0] + input_df['PHOSPHORUS'][0] + input_df['POTASSIUM'][0]) / 3  

    recommendations = []

    for i, feature in enumerate(features):
        if feature in ['SIZE', 'AUI']:  # Area and Irrigation Impact
            if contributions[i] > 0:
                recommendations.append(f"✔️ Increasing {readable_features[feature]} further can improve yield.")
            else:
                recommendations.append(f"🔼 Increase {readable_features[feature]} further for better yield.")

        elif feature in ['NITROGEN', 'PHOSPHORUS', 'POTASSIUM']:  # Fertilizer Impact
            if n_p_k_contribution > 0:
                recommendations.append(f"✔️ Increasing {readable_features[feature]} further can improve yield.")
            else:
                recommendations.append(f"🔼 Increase {readable_features[feature]} intake for better yield.")
...
.combined-fertilizer-impact {
    color:rgb(213, 0, 255); /* Light green */
}
        .darker-box ul {
            color: white;
        }
    ...
    # Generate Combined Fertilizer Impact HTML
    if combined_fertilizer_impact:
        impact_analysis_html += "<h4 class='combined-fertilizer-impact'>Combined Fertilizer Impact 🌱:</h4>"
        impact_analysis_html += "".join(f"<li>{impact['impact_text']}</li>" for impact in combined_fertilizer_impact)
...
if st.session_state.page == 'User Input':
    ...
    if submitted:
        if all(value >= min_values[feature] for feature, value in zip(features, input_data)):
            ...
            if combined_fertilizer_impact:
                impact_html += "<h4 class='combined-fertilizer-impact'>Combined Fertilizer Impact 🪴:</h4>"
                impact_html += "".join(f"<li>{impact['impact_text']}</li>" for impact in combined_fertilizer_impact)
